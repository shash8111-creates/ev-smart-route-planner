...
from energy import estimate_energy_kwh  # Use your updated version without reserve_frac
...
# --- Current battery input as percentage ---
...
def find_chargers_osm(lat, lon, distance_km=30):
    """
    Fetch EV chargers using OpenStreetMap Overpass API (no API key required)
    """
...
if st.button("Plan Route") or st.session_state.route_data:
    try:
        ...
        col2.metric("Distance", f"{st.session_state.route_data['routes'][0]['distance']/1000:.1f} km")
        col3.metric("Duration", f"{st.session_state.route_data['routes'][0]['duration']/60:.1f} min")
