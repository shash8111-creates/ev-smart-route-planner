...
# User Inputs
# ----------------------------
vehicles = ["Tata Nexon EV", "MG ZS EV", "Hyundai Kona EV", "Mahindra eVerito"]
...
vehicle_choice = st.selectbox("Select Vehicle", vehicles)
...
def load_or_train_model():
    ...
    df = pd.read_csv("ev_energy_dataset_full_updated.csv")  # Updated dataset with drive_mode
...
if st.button("Plan Route"):
    ...
    if start_coords and end_coords:
        ...
        st.write(f"Predicted Energy Consumption: **{energy_pred:.2f} kWh**")

        # Map
        m = folium.Map(location=[(start_coords[0]+end_coords[0])/2,(start_coords[1]+end_coords[1])/2], zoom_start=8)
        ...
        route_points = [(lat, lon) for lon, lat in route_data["routes"][0]["geometry"]["coordinates"]]
        folium.PolyLine(route_points, color="blue", weight=4, opacity=0.8).add_to(m)

        # Add chargers near midpoint
        mid_lat = (start_coords[0]+end_coords[0])/2
        mid_lon = (start_coords[1]+end_coords[1])/2
        chargers = find_chargers_osm(mid_lat, mid_lon)
        for lat, lon, name in chargers:
